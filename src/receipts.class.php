...
class jkusdatr_receipts Extends JKUSDATREASURY
{
 ...
 private function __new_delete_receipt()
 {
	...
	$date = isset($vars["date"])?stripslashes($vars["date"]):false;
	$ind = isset($vars["ind"])?stripslashes($vars["ind"]):false;
	...
	$i = 0;
	while($i <= $ind && $row = mysqli_fetch_assoc($check)["Ind"]){$tmp = $row;$i++;}
	$id = $tmp;
	$_GET["id"] = $id;
	$this->__delete_receipt();
	$this->result = 7;
 ...
 private function __new_view_formatted_receipt()
 {
	...
	$date = isset($vars["date"])?stripslashes($vars["date"]):false;
	$ind = isset($vars["ind"])?stripslashes($vars["ind"]):(isset($vars["id"])?stripslashes($vars["id"]):false);
 ...
 private function __get_first_id($date, $table)
 {
	...
	while($row = mysqli_fetch_assoc($check)["Ind"])
	{
		if($count == 0)$prev = $row;
		else
		{
 ...
 private function __upload_receipt_by_date()
 {
	...
	$date = $_GET["date"];
	echo "<script type=\"text/javascript\">/*alert(\"done\");*/window.location.assign(\"http://localhost/treasury/jkusdatreasury/oldckc/?main=receipts&action=upload&date=".$date."\");</script>";
	$this->result = 7; 
